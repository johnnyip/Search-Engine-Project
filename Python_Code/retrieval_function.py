...
class query_retrieval():
    ...
    def __init__(self, body_inverted_index={}, body_forward_index={}, body_df_index={}, header_inverted_index={},
                    header_forward_index = {}, header_df_index = {}, url_list={}, stopword_list={},
                     header_doc_length = {}, body_doc_length = {}, page_rank_index={}):
    ...
    def similarity_score_calculation(self, inverted_index, df_index, doc_length, query):
        ...
        for id, item in enumerate(query_items):
            query_items[id] = test.stopword_removal(inword=item, stopword_list=stopword_list)
        ps = PorterStemmer()
            ...
            if inverted_index.get(item) is not None:
                ...
                for doc in full_doc_set:
                    ...
                    if document_collection.get(doc, 'Null') == 'Null':
                        ...
                        temp_list = dict_to_list(item, inverted_index[item][doc][:-1])
    ...
    def overall_retreival_function(self, query, title_weight=10):
        ...
        if phrase_query != '':
           ...
           header_collection = self.phrase_searching(self.header_inverted_index, self.header_forward_index, phrase_query)
...
if __name__ == '__main__':
    ...
    with open('Body_Inverted_Index.dat', 'rb') as f:
        body_inverted_index = pickle.load(f)
    f.close()
    del f
    with open('Body_Forward_Index.dat', 'rb') as f:
        body_forward_index = pickle.load(f)
    f.close()
    del f
    with open('Body_DF_Index.dat', 'rb') as f:
        body_df_index = pickle.load(f)
    f.close()
    del f
    with open('Header_Inverted_Index.dat', 'rb') as f:
        header_inverted_index = pickle.load(f)
    f.close()
    del f
    with open('Header_Forward_Index.dat', 'rb') as f:
        header_forward_index = pickle.load(f)
    f.close()
    del f
    with open('Header_DF_Index.dat', 'rb') as f:
        header_df_index = pickle.load(f)
    f.close()
    del f
    with open('stopword_list.dat', 'rb') as f:
        stopword_list = pickle.load(f)
    f.close()
    del f
    with open('url_list.dat', 'rb') as f:
        url_list = pickle.load(f)
    f.close()
    del f
    with open('header_doc_length.dat', 'rb') as f:
        header_doc_length = pickle.load(f)
    f.close()
    del f
    with open('body_doc_length.dat', 'rb') as f:
        body_doc_length = pickle.load(f)
    f.close()
    with open('page_rank_index.dat', 'rb') as f:
    ...
    test = WebC.webpage_crawling()
    search = query_retrieval(body_inverted_index, body_forward_index, body_df_index, header_inverted_index,
                             header_forward_index, header_df_index, url_list, stopword_list,
                             header_doc_length, body_doc_length, page_rank_index)
    ...
    print('Query:\t\t\t', query, '\n'
                                 'Phrasal query:\t', str(re.findall('<.*?<', query.replace('"', '<'))[0]).replace('<', ''),
          '\n'
          'Search Result:\t',
